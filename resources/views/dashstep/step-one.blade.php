...
<form method="post" action="{{ route('content.store') }}" class="mt-6 space-y-6">
    ...
    <div><x-text-input id="unid" class="block mt-1 w-full" type="hidden" name="unid" value="{{Auth::user()->id;}}" /></div>
        <div><x-text-input id="unsts" class="block mt-1 w-full" type="hidden" name="unsts" value="one" /></div>

        @if(isset($iduser) and ($spts == "stepone" or $spts == "steptwo" or $spts == "stepthree" or $spts == "stepfour" or $spts == "stepfive" ))
